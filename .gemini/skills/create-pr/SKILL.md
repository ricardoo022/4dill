---
name: create-pr
description: Creates a pull request for the LusitAI aipentest project following the project's PR template and git workflow conventions. Use when asked to "create a PR", "open a PR", "make a pull request", "push and create PR", or when implementation work is complete and ready to submit for review. Handles branch detection, diff analysis, PR body generation from the project template, and `gh pr create` execution.
---
...
2. If the user provides a US ID, create a feature branch: `feature/US-XXX-`
3. If the user indicates this is a non-US change (chore, docs, etc.), ask for the **change type** (chore/docs/fix) and a **short description**, then create: `chore/` or `docs/`
...
```bash
gh pr list --head "" --json number,state --jq '.[0]'
```

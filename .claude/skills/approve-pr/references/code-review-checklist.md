...
- New Jinja2 prompt templates not placed in `src/pentest/templates/prompts/` or not using `.md.j2` extension — template renderer must point `template_dir` to `Path(__file__).parent / "prompts"` and call `env.get_template("name.md.j2")`. Wrong path/extension causes `env.get_template(...)` to raise `TemplateNotFound` at runtime.
